                        ...
                        {result.recommended_doctors.length > 0 && (
                            <section className="mt-6 border-t border-slate-100 pt-5">
                                <div className="mb-3 flex items-end justify-between gap-3">
                                    <div>
                                        <p className="text-[10px] font-bold uppercase tracking-[0.16em] text-teal-700">Matched for your result</p>
                                        <h4 className="mt-1 text-base font-bold text-slate-900">Recommended specialists nearby</h4>
                                    </div>
                                    <span className="rounded-full bg-teal-50 px-2.5 py-1 text-[10px] font-bold text-teal-700">Top matches</span>
                                </div>
                                <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
                                    {result.recommended_doctors.map((doctor) => (
                                        <article key={doctor.doctor_id || doctor.email} className="flex flex-col justify-between rounded-2xl border border-slate-200 bg-slate-50/70 p-4 transition hover:-translate-y-0.5 hover:border-teal-200 hover:bg-white hover:shadow-md">
                                            <div>
                                                <div className="flex items-start justify-between gap-2">
                                                    <div>
                                                        <h5 className="text-sm font-bold text-slate-900">{doctor.name}</h5>
                                                        <p className="mt-1 text-[11px] font-semibold text-teal-700">{doctor.speciality}</p>
                                                    </div>
                                                    {doctor.rating !== null && doctor.rating !== undefined && (
                                                        <span className="inline-flex shrink-0 items-center gap-1 rounded-lg bg-amber-50 px-2 py-1 text-[10px] font-bold text-amber-700">
                                                            <Star className="h-3 w-3 fill-current" /> {doctor.rating}
                                                        </span>
                                                    )}
                                                </div>
                                                <div className="mt-3 space-y-1.5 text-[11px] text-slate-500">
                                                    <p className="flex items-start gap-2"><Building2 className="mt-0.5 h-3.5 w-3.5 shrink-0 text-slate-400" />{doctor.hospital}</p>
                                                    <p className="flex items-start gap-2"><MapPin className="mt-0.5 h-3.5 w-3.5 shrink-0 text-slate-400" />{doctor.area ? `${doctor.area}, ` : ''}{doctor.city}</p>
                                                    <p className="flex items-center gap-2"><span className="font-semibold text-slate-700">{doctor.experience}</span><span>•</span><span>Fee: {doctor.fee}</span></p>
                                                </div>
                                            </div>
                                            {doctor.phone && (
                                                <a href={`tel:${doctor.phone}`} className="mt-4 inline-flex items-center justify-center gap-1.5 rounded-xl border border-teal-200 bg-white px-3 py-2 text-[11px] font-bold text-teal-800 transition hover:bg-teal-50">
                                                    <Phone className="h-3.5 w-3.5" /> Contact specialist
                                                </a>
                                            )}
                                        </article>
                                    ))}
